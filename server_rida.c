...
char* read_file(char *file_name){
	...
	if (file) {
		...
		perror("There is no file present");
	}	
...
char* pull_requests_from_clients(int listening_socket, char *client_request){
	...
	char *out;
	char check[100];
//	check[10] = listening_socket; 
//	strcpy(check, listening_socket);
	sprintf(check, "%d\n", listening_socket);
	printf("value of check %d", check);
	
	if((out = (char *)malloc(strlen(check) + strlen(client_request) + 1)) != NULL)
	{
	   
	   strcpy(out, check);
	   strcat(out, client_request);
	}
	else
	{
	   perror("Error : ");//you don't have enough memory, handle it
	}     
	printf("Concatenated client request :  %s", out);
...
//	server_msg[2000] = "Server is live!\n";
	...
	while(1) {
		...
		pthread_mutex_lock (&m);
			...
			//client_request[1000] = buffer[rem];
			
			char l[100];
			l[100] = buffer[rem];
			printf("kjghkjhfjfk%s\n", l);
			
			printf("YESSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS %s");
			puts(buffer);
			
			char socket_id[10];  
			strncpy(socket_id, client_request, 10); 
			puts(client_request);
			printf("\nWorker Thread ID %d : Client request recieved on socket %s.\n", thread_id, client_request);
			
			 // Parse request from clients
			verification_msg = verify_request(client_request);
				       
			if (sizeof(verification_msg) > 1){
				server_msg = generate_server_response(client_request);
...
int main(int argc, char *argv[])
{
	...
	while(1){ 
		...
		else
		{
			...
			new_request = pull_requests_from_clients(listening_socket, new_request);
			printf("Boss : request sent");
			
			
			if (strlen(new_request) > 0){
				...
				puts("Boss : new request---------------\n");
				int len=strlen(new_request);
				new_request[len-1]='\0';
				printf("Boss : Buffer_len:%d\n",len);
				printf("%s", new_request);
				puts("Boss : new request ends--------------\n");
				...
				num++;
				printf("Boss : safely out\n");
			}
			else
			{
				perror("Boss : not good\n");
			}
